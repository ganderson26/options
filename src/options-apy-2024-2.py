...
###df_new = df.groupby(df['month'])['AMOUNT_RECAST'].transform('sum')


#df_new = df.groupby(df['DATE_RECAST'].dt.year, df['DATE_RECAST'].dt.month,'month').agg({'AMOUNT_RECAST': sum})


##df_new = df.groupby(df['DATE_RECAST'].dt.year, df['DATE_RECAST'].dt.month).agg({'AMOUNT_RECAST': sum})
...
######df.drop_duplicates(subset='monthly_apy', inplace=True)
...
print('Total Gain/Loss=', df['monthly'].sum())    

# Calculate Total APY
# Keep it simple and just use number of months/rows
#total_apy = df['monthly_apy'].sum() / df.shape[0]
