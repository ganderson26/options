# Backtest Weekly Wheel for selected stock
...
#
# If assigned:
# - Suspend selling Puts
# - Sell Calls until stock is called away, then resume selling Puts
...
ATM_SPREAD = '1 SD below ATM and Take Assignment and Sell Calls till Called Away'
...
def filter_data_puts(df):
    ...
    # Get just selected stock and drop any NaN
...
# Filter Data
#
# Get the stock rows for Calls
# ---------------------------------------------------------------------------------
def filter_data_calls(df):
...
# ---------------------------------------------------------------------------------
# Get Calls for expiration date and strike just above
#
# Get the short (sell put bid) for 1 SD
# ---------------------------------------------------------------------------------
def get_calls_strike(df, expiration_trade_date, short_strike):

    #print('strike', short_strike)
    #print('expiration trade date', expiration_trade_date)
    
    ### Get next trade date
    # 1. Convert the date column to datetime objects
    last_trade_date = pd.to_datetime(expiration_trade_date)
    #next_trade_date = last_trade_date + pd.Timedelta(days=7)
    ...
    # 1. Filter for all values greater than target
    greater_than_target = df.loc[(df['lastTradeDate'] == expiration_trade_date) & (df['strike'] > short_strike)]
    #print('greater_than_target', greater_than_target)
    # 2. Find the minimum of those values
    just_above_val = greater_than_target['strike'].min()
    #print('strike just above', just_above_val)

    # 3. Filter the original dataframe for that exact value
    #result = df[df['strike'] == just_above_val]

    result = df[(df['lastTradeDate'] == expiration_trade_date) & (df['strike'] == just_above_val)]
...
# ---------------------------------------------------------------------------------
def calculate_premiums(filtered_puts_df, filtered_calls_df, df):
    ...
    # Recast lastTradeDate to just YYYY-MM-DD for comparison
    # Format as Year-Month-Day string
    filtered_calls_df['lastTradeDate'] = filtered_calls_df['lastTradeDate'].dt.strftime('%Y-%m-%d')
        ...
        if prev_row_1 is not None:
            if (row['lastTradeDate'] != prev_row_1['lastTradeDate']):
                ...
                if prev_row_2 is not None:
                    short_strike_2 = prev_row_2['strike']
                ...
                # If expiratioPrice >= short_stike, you will not be assigned  
                if (SUSPEND_PUTS == False): 
                    ...
                    '''    
                        ...
                        if(SUSPEND_PUTS == False):
                            # Get next Call not this trade date
                            OPTION = 'P'
                            close_cost = 0.00
                            close_net = premium - close_cost
                            calls_bid = 0.00
                    ...
                    if (SHORT_STRIKE is None):
                        SHORT_STRIKE = short_strike_2
                    # Get Calls for the following trade dates starting with expiration_trade_date and strike just above short_strike 
                    # Need to keep call strike.....
                    calls_df = get_calls_strike(filtered_calls_df, prev_row_1['lastTradeDate'], SHORT_STRIKE)
                    #print(calls_df)

                    calls_bid = calls_df.get('bid').iloc[0]
                    calls_strike = calls_df.get('strike').iloc[0]
                    calls_current_price = calls_df.get('currentPrice').iloc[0]
                    #print(calls_current_price)
                    print('calls_strike', calls_strike)

                    close_cost = 0.00
                    premium = 0.00
                    close_net = premium - close_cost

                    if (prev_row_1['currentPrice'] > calls_strike).any():
                        SUSPEND_PUTS = False
                        SHORT_STRIKE = None
                ...
                new_row = {'Option': OPTION, 'contractSymbol': prev_row_1['contractSymbol'], 'lastTradeDate': prev_row_1['lastTradeDate'], 
                    ...
                    'calls_strike': calls_strike, 'calls_bid': calls_bid}
...
def main():
    ...
    filtered_calls_df = filter_data_calls(load_df)

    #spread_expiration_df = get_sd_strike_expiration(filtered_puts_df)
    
    spread_df = get_sd_strike(filtered_puts_df)
    
    premiums_df = calculate_premiums(filtered_puts_df, filtered_calls_df, spread_df)
